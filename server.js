...
// Serverless-safe connection: cache a single connection promise across
// invocations so warm containers reuse it and requests never run against an
// unconnected mongoose. On Vercel each cold start reuses this cached promise.
...
function connectDB() {
  ...
  if (!connectionPromise) {
    connectionPromise = mongoose
      ...
      .catch((err) => {
        // Reset so a later request can retry instead of caching a failure.
...
// One-time data migrations. Run once per warm process, guarded so cold starts
// don't re-run them on every invocation and so a failure never blocks requests.
...
function runStartupMigrations() {
  ...
  Promise.resolve()
    .then(() => {
      ...
      Product.find({ image: /^\/assets\/images\/generated-menu\// })
        .lean()
        .then(async (localProducts) => {
          ...
          for (const p of localProducts) {
            ...
            await Product.updateOne(
              { productId: p.productId },
              { $set: { image: `/api/products/${p.productId}/image` } },
            );
            imported++;
          }
          if (imported > 0)
            console.log(`Imported ${imported} product image(s) into MongoDB`);
        })
      ...
      Promise.all(
        ...
        .then((results) => {
          ...
          if (updated > 0)
            console.log(`Set ${updated} Popular Blend featuredImage URL(s)`);
        })
        .catch((err) =>
          console.error("featuredImage update error:", err.message),
        );
      ...
      Product.find({ isFeatured: true })
        .lean()
        .then(async (featuredProducts) => {
          for (const p of featuredProducts) {
            const hasStoredImage = await ProductImage.findOne({
              productId: p.productId,
            })
              .select("_id")
              .lean();
            if (
              hasStoredImage &&
              p.image !== `/api/products/${p.productId}/image`
            ) {
              await Product.updateOne(
                { productId: p.productId },
                { $set: { image: `/api/products/${p.productId}/image` } },
        ...
        .catch((err) =>
          console.error("Featured image field fix error:", err.message),
        );
    })
    .catch((err) => console.error("Startup migration error:", err.message));
}

// Ensure the DB is connected before any API route runs. On serverless this
// awaits the cached connection per request; on a warm container it resolves
// immediately. Startup migrations kick off once the connection is ready.
...
// Serve static frontend files (HTML, JS, CSS, assets)
...
// Only start a listening server when run directly (local dev / node server.js).
// On Vercel the platform imports the exported app and handles invocation, so
// calling app.listen() there is unnecessary.
